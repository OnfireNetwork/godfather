...
AddEvent("OnPackageStart", function()
    ...
    mariadb_query(db, "SELECT * FROM houses;", function()
        for row=1,mariadb_get_row_count() do
            local hid = getIDHouse(mariadb_get_value_name_int(row, "id"))
            if hid ~= 0 then
                houses[hid].owner = mariadb_get_value_name_int(row, "owner")
                if houses[hid].owner == 0 then
                    houses[hid].textentities = {
                        CreateText3D(_("house_number").." "..houses[hid].id, 10, houses[hid].text[1], houses[hid].text[2], houses[hid].text[3]+20, 0, 0, 0),
                        CreateText3D(_("price")..": "..houses[hid].price.." ".._("currency_symbol"), 10, houses[hid].text[1], houses[hid].text[2], houses[hid].text[3]+5, 0, 0, 0)
                    }
                else
                    mariadb_query(db, "SELECT name FROM players WHERE id='"..houses[hid].owner.."';", function()
                        houses[hid].textentities = {
                            CreateText3D(_("house_number").." "..houses[hid].id, 10, houses[hid].text[1], houses[hid].text[2], houses[hid].text[3]+20, 0, 0, 0),
                            CreateText3D(_("owner")..": "..mariadb_get_value_name(1, "name"), 10, houses[hid].text[1], houses[hid].text[2], houses[hid].text[3]+5, 0, 0, 0)
                        }
                    end)
                end
            end
...
AddEvent("OnPlayerDataReady", function(player, data)
    if data.spawn ~= 0 then
        local house = getIDHouse(data.spawn)
        if house == 0 then
            return
        end
        if houses[house].owner ~= data.db then
...
AddCommand("setspawn", function(player)
    local x, y, z = GetPlayerLocation(player)
    for i=1,#houses do
        if GetDistance3D(x, y, z, houses[i].text[1], houses[i].text[2], houses[i].text[3]) < 200 then
            if houses[i].owner == player_data[player].db then
                player_data[player].spawn = houses[i].id
                AddPlayerChat(player, _("set_spawn_success"))
            else
                AddPlayerChat(player, _("not_your_house"))
            end
            return
        end
    end
